...
from .models import Order, TypeOfDelivery
...
class OrderAddressForm(forms.ModelForm):
    ...
    class Meta:
        model = Order
        fields = (
            'mail', 'first_name', 'last_name', 'country', 'region', 'city', 'street', 'building', 'phone', 'comment'
        )


class OrderDeliveryForm(forms.ModelForm):

    # typeDelivery = TypeOfDelivery.objects.values_list('id', 'title')

    typeDelivery = Order.objects.filter()
    delivery = forms.ChoiceField(choices=typeDelivery, widget=forms.Select, required=False)


    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['delivery'].label = 'Cпособ доставки:'
