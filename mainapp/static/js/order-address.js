...
function init(city_info) {
    ...
    $.ajax({
        url: `http://127.0.0.1:8000/api/cdek-pvz/?format=json&search=${city}`
            ...
            var new_data = data.map(function (pvz) {
                    ...
                    pvz = {
                        ...
                        "properties": {
                            ...
                            "balloonContentBody": `<p>Адрес: ${pvz.address} </p><p> Стоимость доставки: ${cdekPrice} ₽</p> <p>График работы: ${pvz.work_time}</p><input type='button' value='Забрать здесь' name='${pvz.code}'>`,
        ...
        $.ajax({
            url: `http://127.0.0.1:8000/api/cdek-city/?format=json&search=${city}`
                ...
                if (data.length !== 0) {
                    ...
                    $('input[type=radio][name=delivery_choice][value=cdek]').change(function () {
                        if (this.value === '1') {
                            alert('Сработало')
                    ...
                    var options =
                        {
                            fields:
                                [
                                    {id: 'id_region', levels: ['Region', 'District']},
                                    {
                                        id: 'id_city',
                                        levels: ['City', 'Place'],
                                        suggest_on_empty: true
                                    },
                                    {id: 'id_street', levels: ['Site', 'Street']},
                                    {id: 'id_building', levels: ['House', 'Building', 'Structure', 'Flat']},
                                ],
                            limit: 7,
                            user: 'demotoken',
